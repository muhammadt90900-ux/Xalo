...
def fib_webhook():
    ...
    if status in ('PAID', 'paid', 'success') and payment_id:
        # گەڕان بەدوای بەکارهێنەر بە payment_id لە session یان لە pending_order_id
        # order_id شێوازی "FIB-{user_id}-{timestamp}" هەیە
        # بەمەش دەتوانین user_id بدۆزینەوە ئەگەر بە ئەم شێوازە پاشەکەوت کرابێت
        user = None
        # گەڕان بەدوای هەموو بەکارهێنەرانی VIP نەچالاک
        users_pending = User.query.filter(
            User.plan != 'vip'
        ).all()
        # چونکە FIB payment_id ئەڵاقەی بە user_id نییە بەڕاستەوخۆ،
        # پێویستە پارەدان لە API پشکنین بکەین
        try:
            from fib_payments import FIBPaymentsClient, FIBConfig
            import asyncio
            ...
            pstatus = asyncio.run(_check())
            logger.info(f"FIB webhook payment verify: {pstatus}")

            if pstatus and pstatus.get('status') in ('PAID', 'paid', 'success'):
                # دۆزینەوەی user_id لە description
                desc = pstatus.get('description', '')
                for u in users_pending:
                    if u.username in desc:
                        activate_vip_for_user(u)
                        logger.info(f"VIP activated via FIB webhook for user: {u.username}")
                        break
        except Exception as e:
            logger.error(f"FIB webhook verify error: {e}")
...
@login_required
def payment_success():
    """لاپەڕەی سەرکەوتنی پارەدان — تەنها پەیامی پشتڕاستکردنەوە نیشان دەدات"""
    gateway = request.args.get('gateway', '')

    # ئەمنی: VIP تەنها لە ڕێگەی webhook/IPN ی gateway دەکرێتەوە
    # ئەمجا هیچ کەسێک نەتوانێت بەبێ پارەدانی ڕاستەقینە VIP بگرێت
    if is_vip_active(current_user):
        flash('پارەدانەکەت پشتڕاست کرایەوە! ئێستا VIPـیت چالاکە. 🎉', 'success')
    else:
        flash('پارەدانەکەت وەرگیرا و لەلایەن سیستەمەوە پشکنینی دەکرێت. چەند خولەک بوەستەرەوە. ⏳', 'info')
...
def admin_delete_user(user_id):
    user = db.session.get(User, user_id)
    if user and user.user_type != 'admin':
        # سڕینەوەی هەموو داتای پەیوەندیدار بۆ دواگرتنی constraint error
        # سەرەتا reviewەکانی پارچەکانی ئەم بەکارهێنەرە
        part_ids = [p.id for p in Part.query.filter_by(seller_id=user.id).all()]
        if part_ids:
            Review.query.filter(Review.part_id.in_(part_ids)).delete(synchronize_session=False)
        # reviewەکانی خودی ئەم بەکارهێنەرە لە پارچەی کەسانی تر
...
def search():
    ...
    if sort == 'price_asc':
        ...
        parts = query.all()
        parts.sort(key=lambda p: p.avg_rating, reverse=True)
